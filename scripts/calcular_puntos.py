...
def puntuar_partido_elim(pronostico_str, resultado_str, ganador_real=None, equipo_predicho_pasa=None):
    ...
    Reglas:
      ...
      - Marcador exacto sin empate: 4 pts.
      - Marcador exacto con empate (penaltis):
          · Acertó quién pasa: 4 pts.
          · No acertó quién pasa: 1 pt.
    """
...
def encontrar_equipo_predicho(nombre_partido, lista_clasificados):
    """De los dos equipos del partido, devuelve cuál aparece en la lista
    de clasificados del participante para esa ronda."""
    if not lista_clasificados:
        return None
    equipos = nombre_partido.split("-")
    if len(equipos) != 2:
        return None
    lista_norm = [e.strip() for e in lista_clasificados]
    for eq in equipos:
        if eq.strip() in lista_norm:
            return eq.strip()
    return None
    ...
    for excel_path in excels:
        ...
        for pos_key, equipo_real in clasificados_grupo.items():
            ...
            if pronostico and pronostico.strip() == equipo_real.strip():
                ...
                detalle_extra.append({
                    "concepto": f"Clasificado: {pos_key}",
                    "equipo":   equipo_real,
                    "puntos":   PTS_CLASIFICADO
                })
        ...
        for ronda, equipos_reales in eliminatorias.items():
            if not equipos_reales:
                continue
            # Dieciseisavofinalista ya se cuenta en clasificados de grupo
            if ronda == "Dieciseisavofinalista":
                continue
            ...
            for equipo_predicho in lista_predicha:
                if equipo_predicho.strip() in equipos_reales_norm:
                    ...
                    detalle_extra.append({
                        "concepto": ronda,
                        "equipo":   equipo_predicho,
                        "puntos":   PTS_ELIMINATORIA
                    })
